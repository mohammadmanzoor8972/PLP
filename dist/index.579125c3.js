document.addEventListener("DOMContentLoaded", function() {
    ...
    if (productName) productName.addEventListener("click", ()=>{
        window.location.href = "./index.html";
    });
    const shopNowBtn = document.getElementById("shopNowBtn") || document.getElementById("productHome");
    ...
    function updatePrices() {
        ...
        if (minValue > maxValue - 50) {
            if (event.target.id === "minPrice") minPriceSlider.value = maxValue - 50;
            else maxPriceSlider.value = minValue + 50;
        }
    ...
    function updateTotalProductCount() {
        ...
        if (totalProductCountElement) {
            const renderedProductsCount = productsContainer.querySelectorAll(".productList").length;
    ...
    function sortProducts(sortBy) {
        if (sortBy === "Price -- Low to High") allProducts.sort((a, b)=>a.price - b.price);
        else if (sortBy === "Price -- High to Low") allProducts.sort((a, b)=>b.price - a.price);
        else if (sortBy === "Ratings") allProducts.sort((a, b)=>b.rating.rate - a.rating.rate);
    ...
    if (window.location.pathname.includes("productDetails.html")) productDetails();
});
